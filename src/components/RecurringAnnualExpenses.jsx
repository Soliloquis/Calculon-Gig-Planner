...
export default function RecurringAnnualExpenses({
  ...
  const getMonthName = (mVal) => {
    return MONTHS.find(m => m.value === mVal)?.label || 'Jan';
  };
  ...
  return (
    <div className="glass-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      {/* Form Row */}
      <form onSubmit={handleSubmit} className="quick-add-form" style={{ gridTemplateColumns: '1fr 140px 100px 120px 70px 140px 110px', gap: '8px', padding: '0', background: 'transparent', border: 'none' }}>
        <div className="form-group">
          <label>Description</label>
          <input
            type="text"
            className="form-input"
            placeholder="e.g. LLG Annual Franchise Tax"
            value={description}
      ...
      {/* Active Items Row */}
      {recurringAnnualExpenses.length > 0 ? (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '4px' }}>
          {recurringAnnualExpenses.map(item => {
            ...
            return (
              ...
              >
                ...
                <span style={{ color: 'var(--text-muted)' }}>
                  ({currencySymbol}{item.plannedAmount}/yr, {getMonthName(item.month)} {item.day}{item.endsAt ? `, ${formatEndMonth(item.endsAt)}` : ''})
                </span>
